 ...
 * create a single handler (e.g., Default_Handler()) for all the exceptions and allow the
 * programmer to implement the required handlers as per the application requirements.
 *
 * The 'alias' attribute allows programmer to give alias for a function. Whenever
 * NMI_Handler() gets called, Default_Handler() will be executed. In other words, the
 * address of Default_Handler will be stored in the NMI_Handler's place in the vector
 * table.
 *
 * The 'weak' attribute allows programmer to override already defined weak function
 * (dummy) with the same function name. For example, if your project requires the
 * implementation of the NMI_Handler(), simply define the function without modifying this
 * function declaration. Compiler will find the definition of the defined function, and
 * the defined function will be executed at run-time instead of the dummy function, the
 * Default_Handler().
 */
...
uint32_t vectors[] __attribute__((section(".isr_vector"))) = {
	...
	0,									/* Reserved fields must be set to 0 */
	0,									/* Reserved fields must be set to 0 */
	0,									/* Reserved fields must be set to 0 */
	0,									/* Reserved fields must be set to 0 */
